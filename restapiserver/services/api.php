	...
	class API extends REST {
		...
		*/
		private function dbConnect(){
			$this->mysqli = new mysqli(self::DB_SERVER, self::DB_USER, self::DB_PASSWORD, self::DB, 3306);
		}
		...
		private function xmlcustomers(){	
			...
			$res = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
                        ...
                        while ($row = $res->fetch_assoc()) {
                             $item_record = array(
                                            '@attributes' => array ('id' => $row['customerNumber']),
                                            'username' => $row['customerName'],
                                            'address' => $row['address'],
                                            'email'=> $row['email']
                                        );
					$result['customer'][] = $item_record;
                        }
                        //$this->response($this->json($result), 200);
                        $this->xmlresponse(Array2XML::createXML('customerlist', $result)->saveXML(), 200); 
                        
                    }
			$this->response('',204);	// If no records "No Content" status
		}
                
                private function xmlcustomer(){	
			if($this->get_request_method() != "GET"){
				$this->response('',406);
			}
		...
		private function insertCustomer(){
			...
			if(!empty($customer)){
                ...
                private function xmlinsertCustomer(){
			if($this->get_request_method() != "POST"){
				$this->response('',406);
			}

			$column_names = array('customerName', 'email','address');
			...
			if(!empty($_POST)){
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				$success = array('status' => "Success", "msg" => "Customer Created Successfully.", "data" => $_POST);
				$this->response($this->json($success),200);
			}else
				$this->response('',204);	//"No Content" status
		}
                private function xmlupdateCustomer(){
...
//			 exit('sf');
			...
			if(!empty($_POST)){
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				$success = array('status' => "Success", "msg" => "Customer ".$id." Updated Successfully.", "data" => $_POST);
				$this->response($this->json($success),200);
			}else
				$this->response('sfsdfs',204);	// "No Content" status
		}
                
                private function xmldeleteCustomer(){
